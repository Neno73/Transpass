...
export const getUserProducts = async (userId: string) => {
  try {
    ...
    try {
      ...
      console.log(
        `Query executed successfully, found ${querySnapshot.size} documents`
      );
      ...
      if (
        ...
        console.warn(
          "Index error detected - this is normal when first setting up the database"
        );
...
export const getCompanyScanAnalytics = async (companyId: string) => {
  try {
    ...
    const productAnalytics = Object.keys(productScans)
      .map((productId) => ({
        productId,
    ...
    scanHistory.forEach((scan) => {
      if (scan.scannedAt) {
        ...
        if (
          ...
          typeof scan.scannedAt.toDate === "function"
        ) {
...
export const createProduct = async (
  ...
  try {
    ...
    if (imageFile) {
      try {
        ...
        const storageRef = ref(
          storage,
          `product-images/${timestamp}_${safeFileName}`
        );
        ...
        while (uploadAttempts < maxAttempts && !success) {
          try {
            uploadAttempts++;
            console.log(
              `Upload attempt ${uploadAttempts} for image, bytes:`,
              imageFile.size
            );
    ...
    if (productData.components && productData.components.length > 0) {
      console.log(
        `Processing ${productData.components.length} components for documents`
      );
  ...
  try {
    ...
    if (imageFile) {
      try {
        ...
        const storageRef = ref(
          storage,
          `product-images/${Date.now()}_${safeFileName}`
        );
...
/**
 * Fetches all products for a specific company
